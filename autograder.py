...
def check_perceptron(tracker):
    ...
    for dimensions in range(1, 10):
        ...
        for point in (random_point, np.zeros_like(random_point)):
            ...
            expected_prediction = np.where(np.dot(point, p.get_weights().data.T.numpy()) >= 0, 1, -1).item()
    ...
    accuracy = np.mean(np.where(np.dot(dataset.x, model.get_weights().data.T.numpy()) >= 0.0, 1.0, -1.0) == dataset.y)
    if accuracy < 1.0:
